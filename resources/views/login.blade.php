...
<body class="font-bold font-sans">
            ...
            @if ($errors->any())
                <ul class="flex flex-col absolute w-full gap-y-1">
                    @foreach ($errors->all() as $error)
                        <li class="flex inset-x-12 px-3 py-1 gap-2 bg-violet-500" id={{"toast".$loop->index}}>
                            ...
                            <button class="w-fit h-fit ml-auto" onclick="onToastClose({{$loop->index}})">
    ...
    <script>
        function onToastClose(errId) {
            document.getElementById("toast"+errId).remove()
        }
    </script>
</body>
</html>