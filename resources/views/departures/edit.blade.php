        ...
        <div class="w-full h-full relative">
            <div class="absolute w-full h-full lg:px-32 py-16">
                <div class="flex flex-col items-start w-full h-full py-8 bg-white shadow-2xl font-semibold">
                    <form action="{{ route('departures.update', $departure->id) }}" method="POST" enctype="multipart/form-data" class="w-5/6 h-full flex flex-col mx-auto my-auto">
                        @csrf
                        @method('PUT')
                        <div class="h-1/5">
                            ...
                            <h1 class="w-fit mx-auto text-xl lg:text-2xl">Editar Saída</h1>
                        </div>
                        <div class="flex flex-col my-auto gap-y-12 lg:gap-y-20 font-normal text-lg lg:text-lg">
                            <div class="lg:w-full flex flex-col lg:flex-row gap-8">
                                <div class="lg:w-1/2 flex flex-col gap-y-10">
                                    <div class="relative flex flex-col">
                                        ...
                                        <input type="number" name="student_rm" id="student_rm" value="{{ $departure->student_rm }}"  class="px-2 lg:px-4 py-1 lg:py-2 border-2 border-slate-400 rounded-sm" />
                                    ...
                                    <div class="relative flex flex-col">
                                        <label for="departure_time" class="font-semibold">Tempo de chegada: </label>
                                        <input type="datetime-local" name="departure_time" id="departure_time" value="{{ $departure->departure_time }}" class="px-2 lg:px-4 py-1 lg:py-2 border-2 border-slate-400 rounded-sm">
                                        <span class="absolute top-full">
                                ...
                                <div class="lg:w-1/2 relative flex flex-col">
                                    ...
                                    <textarea name="reason" id="reason" class="h-full px-2 lg:px-4 py-1 lg:py-2 border-2 border-slate-400 rounded-sm">{{ $departure->reason }}</textarea>
                                    <span class="absolute top-full">
                                        @error('reason')
                                            {{$message}}
                                        @enderror
                            ...
                            <div class="flex flex-col lg:flex-row gap-x-8 gap-y-2 font-semibold">
                                ...
                                <a href="/departures" class="grow py-1 lg:py-2 border-2 border-slate-400 rounded-sm text-center">Voltar</a>
                            </div>
                        </div>
                    </form>
                </div>
